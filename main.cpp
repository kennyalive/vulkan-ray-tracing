...
void CreateCommandBuffers(uint32_t queueFamilyIndex)
{
    ...
    VkResult result = vkCreateCommandPool(vkApp.device, &commandPoolCreateInfo, nullptr, &vkApp.presentQueueCommandPool);
...
void CreateSemaphores()
{
    ...
    VkResult result = vkCreateSemaphore(vkApp.device, &semaphoreCreateInfo, nullptr, &vkApp.imageAvailableSemaphore);
...
void CreateVulkanResources(HWND windowHandle)
{
    vkApp.instance = CreateInstance();
    VkPhysicalDevice physicalDevice = SelectPhysicalDevice(vkApp.instance);

    vkApp.surface = CreateSurface(vkApp.instance, windowHandle);

    DeviceInfo deviceInfo = CreateDevice(physicalDevice, vkApp.surface);
    vkApp.device = deviceInfo.device;
    vkApp.graphicsQueue = deviceInfo.graphicsQueue;
    vkApp.presentQueue = deviceInfo.presentationQueue;
...
int main()
{
    ...
    CreateVulkanResources(wmInfo.info.win.window);
    RunMainLoop();
